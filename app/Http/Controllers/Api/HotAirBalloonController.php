...
class HotAirBalloonController extends Controller
{
	...
	$to_date = $request->created_at;
        ...
        $query = "SELECT balloon_bet.*, 
            balloon_bet.win AS cashout_amount,
            balloon_bet.multiplier AS multiplier,
            balloon_result.price AS crash_point 
          FROM balloon_bet 
          LEFT JOIN balloon_result 
          ON balloon_bet.game_sr_num = balloon_result.game_sr_num";
        if (!empty($where)) {
